'use client'

import { useState, useEffect } from 'react'
import Link from 'next/link'
import { WHATSAPP_NUMBER } from '@/shared/utils/constants'
import { storeService, StoreInfo } from '@/features/store/services/storeService'

export default function Footer() {
  const [storeData, setStoreData] = useState<StoreInfo | null>(null)
  const year = new Date().getFullYear()

  useEffect(() => {
    storeService.getStoreInfo().then(setStoreData).catch(() => {})
  }, [])

  const storeName = storeData?.storeName ?? 'Sisi'
  const { hours, contact } = storeData ?? { hours: {}, contact: { address: '', phone: '', email: '' } }

  return (
    <footer>
      <div className="footer-container">
        <div className="footer-section">
          <h3>{storeName}</h3>
          <p className="footer-description">Productos importados en tendencia. Belleza, tecnología, hogar, infantil y más. Envíos a todo Lima y Perú.</p>
          <div className="social-links" style={{ justifyContent: 'flex-start' }}>
            <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" className="social-icon" title="WhatsApp" aria-label="WhatsApp" style={{ color: '#25d366', fontSize: '1.5rem' }}>
              <svg viewBox="0 0 24 24" fill="currentColor" width="24" height="24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347"/></svg>
            </a>
            <a href="https://facebook.com/sisi.pe" target="_blank" className="social-icon" title="Facebook" aria-label="Facebook" style={{ color: '#1877f2', fontSize: '1.5rem' }}>
              <svg viewBox="0 0 24 24" fill="currentColor" width="24" height="24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a href="https://instagram.com/sisi.pe" target="_blank" className="social-icon" title="Instagram" aria-label="Instagram" style={{ fontSize: '1.5rem' }}>
              <svg viewBox="0 0 24 24" fill="currentColor" width="24" height="24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
            </a>
          </div>
        </div>

        <div className="footer-section">
          <h3>Enlaces Rápidos</h3>
          <ul className="footer-links">
            <li><Link href="/">Inicio</Link></li>
            <li><Link href="/productos">Productos</Link></li>
            <li><Link href="/productos?category=Belleza">Belleza</Link></li>
            <li><Link href="/productos?category=Tecnolog%C3%ADa">Tecnología</Link></li>
            <li><Link href="/productos?category=Hogar">Hogar</Link></li>
            <li><Link href="/productos?category=Infantil">Infantil</Link></li>
            <li><Link href="/productos?category=Otros">Otros</Link></li>
          </ul>
        </div>

        <div className="footer-section">
          <h3>Horarios y Contacto</h3>
          <ul className="footer-contact">
            <li>
              <span className="contact-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
              </span>
              <span>{contact.address}</span>
            </li>
            <li>
              <span className="contact-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
              </span>
              <span>{contact.phone}</span>
            </li>
            <li>
              <span className="contact-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="16" height="16"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
              </span>
              <a href={`mailto:${contact.email}`} className="contact-link">{contact.email}</a>
            </li>
          </ul>
          <ul className="footer-hours">
            {Object.entries(hours).map(([key, day]) => (
              <li key={key} className="hour-row">
                <span className="hour-day">{day.day}</span>
                <span className={`hour-value${!day.isOpen ? ' closed' : ''}`}>{day.isOpen ? day.hours : 'Cerrado'}</span>
              </li>
            ))}
          </ul>
        </div>
      </div>

      <div className="footer-bottom">
        <p>&copy; {year} {storeName}. Todos los derechos reservados.</p>
        <p className="creator">Hecho con 💕 por <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank">Sisi</a></p>
      </div>
    </footer>
  )
}
